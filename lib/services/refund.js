...
async function initiateRefund({ gid, merchantTxnId,refundType, paymentData }, config) {
  ...
  try {
   ...
   if (!response) {
      logger.error('Empty response from redundApi ');
      throw new Error('Empty response from redundApi');
    }
