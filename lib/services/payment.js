...
async function initiateApiKeyPayment(payload, config) {
 ...
 try {
  ...
  try {
    ...
    logger.logPaymentOperation('initiateApiKeyPayment', merchantTxnId, result);
    return result;
  } catch (error) {
...
async function initiateJwtPayment(payload, config) {
 ...
 try {
  ...
  logger.debug('JWE and JWS tokens created successfully');
  ...
  try {
    ...
    const result = { paymentLink: redirectUrl, statusLink: statusUrl };
    logger.logPaymentOperation('initiateJwtPayment', merchantTxnId, result);
    return result;
  } catch (error) {
